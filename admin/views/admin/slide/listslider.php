							...
							<?php 
								...
								foreach($sliderList AS $eachRow)
								{
									...
									<tr>
										...
										<td class="center">
											<a target="_blank" href="'. yii::$app->params['SLIDES_DIRECTORY'] . $eachRow['slider_file'] .'"> 
												<img src="'. yii::$app->params['SLIDES_DIRECTORY'] . $eachRow['slider_file'] .'" class="img-rounded" height="50px" width="160px" />
											</a>
										</td>
										...
										<td class="center">
											...
											<a data-id="'. $eachRow['id'] .'" class="btn btn-danger btn-xs deleteButton" href="#deleteModal" style="width: 76px;" data-toggle="modal">
                                                ...
                                                <form method="post" action="" id="form_slider">
                                                    <input type="hidden" name="_csrf" value="'.Yii::$app->request->getCsrfToken().'"/>
                                                    <input type="hidden" name="id_slider"  />
                                                </form>

											</a>
												<form action="'. Yii::$app->request->baseUrl.'/'.md5('admin_updateslider').'" method="post"  style="display: inline;">
                                                    <input type="hidden" name="_csrf" value="'.Yii::$app->request->getCsrfToken().'"/>
													<input type="hidden" name="id" value="'. $eachRow['id'] .'"/>
													<button name="try_edit" class="btn btn-warning btn-xs" style="width: 76px;" type="submit">
														<i class="fa fa-pencil-square"></i> Modifer
													</button>
												</form>
											</div>
										</td>
...
<div class="modal small fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-content">
			...
			<div class="modal-body">
				<h5>Ete-vous sure de supprimer? </h5>
			</div>
			<div class="modal-footer">
				<button class="btn btn-default"data-dismiss="modal" aria-hidden="true"> Retour </button> 
				<a href="#" class="btn btn-danger" id="modalDelete"> Supprimer </a>
			</div>
		</div>
	</div>
</div>
